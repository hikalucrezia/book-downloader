    ...
    while True:
        ...
        if event == "Exit" or event == sg.WIN_CLOSED:
            ...
            if requestSender.checkValidations(values):
                ...
                firstpage, finalpage = int(
                    values["-firstpage"]), int(values["-finalpage"])
                for i in range(firstpage, finalpage+1):
                    responce = requestSender.sendRequest(values, i, ranStr)
                    if responce == None:
                        sg.popup_error(
                            "Error code: 2-02; Responce not returned at all. Try reloading the page and follow the instructions again")
                        break
                    elif not re.match(imgPattern, responce.text, re.S) == None:
                        imgUrl = "https://elib.maruzen.co.jp" + \
                            re.match(imgPattern, responce.text, re.S).group(1)
                        ...
                        if res.status_code == 200:
                            ...
                            sg.popup_error(
                                "Error code: 2-03; Responce denied. See if you are restricted from the website. If you can still access the website from your browser, then try reloading the page and follow the instructions again")
                            break
            ...
            window["-foldername"].update(
                layouts.getRanNumAndToStr(10000, 99999))
    window.close()
